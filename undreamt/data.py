...
class TreeReader:
    ...
    def _fill_cache(self):
        ...
        while len(self.cache) < self.cache_size:
            ...
            src_length = self.get_tree_length(src)
            trg_length = self.get_tree_length(trg)
            if src == '' and trg == '':
                self.epoch += 1
    ...
    def next_batch(self, size):
        ...
        while len(indices) < size:
            try:
                ...
                candidates = [(self._score_length(k[0], k[1], src_min, src_max, trg_min, trg_max), k) for k, v in self.length2pending.items() if get_tree_length(v) > 0]
